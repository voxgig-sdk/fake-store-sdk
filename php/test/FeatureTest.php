...
class FtCtrl extends FakeStoreControl
{
...
class FtClient
{
    ...
    private FakeStoreUtility $utility;

    public function __construct(FakeStoreUtility $utility, array $options)
    {
    ...
    public function get_utility(): FakeStoreUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FakeStoreContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FakeStoreContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FakeStoreContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FakeStoreConfig::make_feature($name);
    ...
    private static function buildUrl(FakeStoreSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FakeStoreError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FakeStoreResponse($fetched);
        }
    ...
    private function fail(FakeStoreContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FakeStoreContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
