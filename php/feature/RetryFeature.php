...
class FakeStoreRetryFeature extends FakeStoreBaseFeature
{
    ...
    public function init(FakeStoreContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FakeStoreContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FakeStoreContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FakeStoreContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
