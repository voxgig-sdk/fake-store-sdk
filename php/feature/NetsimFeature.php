...
class FakeStoreNetsimFeature extends FakeStoreBaseFeature
{
    ...
    private function _simulate(FakeStoreContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FakeStoreContext $ctx, array $applied): void
    {
        $client = $this->client;
