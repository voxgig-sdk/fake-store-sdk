...
class FakeStoreSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FakeStoreHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FakeStoreHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FakeStoreFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FakeStoreFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FakeStoreUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FakeStoreHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FakeStoreHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FakeStoreHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FakeStoreError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FakeStoreHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FakeStoreError("graphql_error",
                "FakeStoreSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
