...
[MCP](https://modelcontextprotocol.io) server exposing the FakeStore SDK as
two agent tools — `fake-store_list` and `fake-store_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-store-mcp)
make build

# 2. Provide credentials via the environment
export FAKE_STORE_APIKEY=sk_live_xxx
...
```jsonc
// fake-store_list: first page of records
{ "entity": "cart" }
{ "entity": "cart", "query": { } }

// fake-store_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-store-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fake-store \
     -- "$PWD"/dist/*/fake-store-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fake-store_list` and `fake-store_load` tools now appear
   in new sessions. Ask the agent to *"list cart using fake-store"*
   and it calls `fake-store_list` with `{"entity":"cart"}`.
...
```sh
export FAKE_STORE_APIKEY=sk_live_xxx            # API key
export FAKE_STORE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fake-store_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fake-store_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FAKE_STORE_APIKEY` | API key sent with every request. |
| `FAKE_STORE_BASE` | Optional override of the API base URL. |
