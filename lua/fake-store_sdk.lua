...
local FakeStoreSDK = {}
FakeStoreSDK.__index = FakeStoreSDK
...
FakeStoreSDK._make_feature = _make_feature


function FakeStoreSDK.new(options)
  local self = setmetatable({}, FakeStoreSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FakeStoreSDK:options_map()
...
function FakeStoreSDK:get_utility()
  return Utility.copy(self._utility)
end


function FakeStoreSDK:get_root_ctx()
  return self._rootctx
end


function FakeStoreSDK:prepare(fetchargs)
...
function FakeStoreSDK:direct(fetchargs)
...
-- by passing it.
function FakeStoreSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FakeStoreSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FakeStoreSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FakeStoreSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FakeStoreSDK.new(sdkopts)
  sdk.mode = "test"
