...
func TestLoginEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FAKE_STORE_TEST_LOGIN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func loginBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FAKE_STORE_TEST_LOGIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FAKE_STORE_TEST_LOGIN_ENTID"])
