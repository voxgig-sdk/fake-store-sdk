...
import (
	...
	omni "github.com/voxgig-sdk/fake-store-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FakeStoreError); is {
				out := map[string]any{
					"name":    "FakeStoreError",
...
func omniCtx(arg any, client *sdk.FakeStoreSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FakeStoreSDK); is {
			client = live
