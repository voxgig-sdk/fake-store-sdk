# FakeStore Golang SDK Reference

Complete API reference for the FakeStore Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *FakeStoreSDK`
...
#### `Cart(data map[string]any) FakeStoreEntity`

Create a new `Cart` entity instance. Pass `nil` for no initial data.

#### `Login(data map[string]any) FakeStoreEntity`

Create a new `Login` entity instance. Pass `nil` for no initial data.

#### `Product(data map[string]any) FakeStoreEntity`

Create a new `Product` entity instance. Pass `nil` for no initial data.

#### `User(data map[string]any) FakeStoreEntity`
...
```go
result, err := client.Cart(nil).Update(map[string]any{
    "id": 1,
    // Fields to update
}, nil)
if err != nil {
...
```go
result, err := client.Cart(nil).Remove(map[string]any{"id": 1}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
...
```go
result, err := client.Product(nil).Remove(map[string]any{"id": 1}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
