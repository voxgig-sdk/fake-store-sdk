// Command FakeStore-cli is a CLI + REPL for the FakeStore SDK,
...
const prompt = "fake-store"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cart login product user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FAKE_STORE_APIKEY (api key), FAKE_STORE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FAKE_STORE_APIKEY carries the API key and
	// FAKE_STORE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FAKE_STORE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FAKE_STORE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFakeStoreSDK(opts)
