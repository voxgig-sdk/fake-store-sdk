...
boru-driven command-line client **and** interactive REPL for the FakeStore
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-store-cli)
make build
...
./fake-store-cli load 1 cart            # {id:1} shorthand
./fake-store-cli load '{id:1}' cart       # explicit match map
./fake-store-cli update '{name:"x"}' cart
./fake-store-cli list login

# 5. Override the API base URL for a single call
FAKE_STORE_BASE=https://api.example.com ./fake-store-cli list cart

# 6. No arguments -> interactive REPL
./fake-store-cli
fake-store> list cart
fake-store> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-store-cli
   ```
...
```sh
./fake-store-cli load 1 cart          # scalar shorthand for {id:1}
./fake-store-cli load '{id:1}' cart     # explicit match map
```
...
```sh
./fake-store-cli update '{id:1,name:"new"}' cart
```
...
Run with no arguments to open a REPL (prompt `fake-store>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fake-store-cli`. |
...
(`./fake-store-cli <expr>`) and interactively (the REPL), and expressions compose the
