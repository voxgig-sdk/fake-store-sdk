...
{
    private static function utility(): FakeStoreUtility
    {
        return new FakeStoreUtility();
    }
    ...
    private static function ctx(array $over = []): FakeStoreContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FakeStoreContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FakeStoreOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FakeStoreError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FakeStoreOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FakeStoreMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FakeStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FakeStoreMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FakeStoreOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FakeStoreMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FakeStoreSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FakeStoreMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FakeStoreResponse([]),
            'result' => new FakeStoreResult([]),
        ]);
        [, $err] = FakeStoreMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FakeStoreSpec([]),
            'response' => null,
            'result' => new FakeStoreResult([]),
        ]);
        [, $err] = FakeStoreMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FakeStoreSpec(['step' => 's']),
            'response' => new FakeStoreResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FakeStoreResult([]),
        ]);
        [, $err] = FakeStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FakeStoreSpec(['step' => 's']),
            'response' => new FakeStoreResponse(self::resp(200, ['v' => 1])),
            'result' => new FakeStoreResult([]),
        ]);
        [, $err] = FakeStoreMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FakeStoreSpec(['step' => 's']),
            'response' => new FakeStoreResponse(self::resp(200, ['v' => 2])),
            'result' => new FakeStoreResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FakeStoreMakeResponse::call($ctx);
    ...
    {
        $preset = new FakeStoreResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FakeStoreSpec([]),
            'response' => new FakeStoreResponse([]),
            'result' => new FakeStoreResult([]),
        ...
        [$got, $err] = FakeStoreMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FakeStoreResult([])]);
        [, $err] = FakeStoreMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FakeStoreSpec([]), 'result' => null]);
        [, $err] = FakeStoreMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FakeStoreSpec(['step' => 's']),
            'result' => new FakeStoreResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FakeStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FakeStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FakeStoreSpec(['step' => 's']),
            'result' => new FakeStoreResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FakeStoreOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FakeStoreMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FakeStoreResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FakeStoreSpec([]),
            'result' => new FakeStoreResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FakeStoreMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FakeStoreMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FakeStoreError('boom', 'boom');
        $utility->fetcher = function (FakeStoreContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FakeStoreSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FakeStoreMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FakeStoreContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FakeStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FakeStoreResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FakeStoreContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FakeStoreSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FakeStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FakeStoreResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FakeStoreSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FakeStoreMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FakeStoreMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FakeStoreMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FakeStoreResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FakeStoreDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FakeStoreResult(['ok' => false])]);
        $this->expectException(FakeStoreError::class);
        FakeStoreDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FakeStoreResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FakeStoreMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FakeStoreMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FakeStoreBaseFeature();
        $b = new FakeStoreBaseFeature();
        FakeStoreFeatureAdd::call($ctx, $a);
        FakeStoreFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FakeStoreBaseFeature
    {
        $f = new FakeStoreBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FakeStoreFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FakeStoreContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FakeStoreSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FakeStorePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FakeStoreResponse(['headers' => null]),
            'result' => new FakeStoreResult([]),
        ]);
        FakeStoreResultHeaders::call($ctx);
