...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FAKESTORE_TEST_LIVE ||
    'TRUE' === process.env.FAKESTORE_TEST_OVERRIDE
  ) {
  ...
  m.FAKESTORE_TEST_EXPLAIN = process.env.FAKESTORE_TEST_EXPLAIN || m.FAKESTORE_TEST_EXPLAIN
