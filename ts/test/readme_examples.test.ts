...
//     (\`new FakeStoreSDK(...)\` / \`FakeStoreSDK.test(...)\` become
//     \`FakeStoreSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cart":{"test01":{"id":"test01"}},"login":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FakeStoreSDK(...)\` and any
// \`FakeStoreSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FakeStoreSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FakeStoreSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
