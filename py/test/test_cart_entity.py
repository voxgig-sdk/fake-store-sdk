    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FAKE_STORE_TEST_CART_ENTID JSON to run live")
        client = setup["client"]
        ...
        cart_ref01_data = helpers.to_map(runner.entity_data(cart_ref01_ent.create(cart_ref01_data, None)))
        ...
        cart_ref01_resdata_up0 = helpers.to_map(runner.entity_data(cart_ref01_ent.update(cart_ref01_data_up0_up, None)))
...
def _cart_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FAKE_STORE_TEST_CART_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FAKE_STORE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("FAKE_STORE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FAKE_STORE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
