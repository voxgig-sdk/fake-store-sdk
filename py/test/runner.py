...
class FakeStoreTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FakeStoreTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FakeStoreTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FakeStoreTestRunner.getenv("FAKE_STORE_TEST_LIVE")
        override = FakeStoreTestRunner.getenv("FAKE_STORE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FakeStoreTestRunner.getenv(key)
        ...
        explain = FakeStoreTestRunner.getenv("FAKE_STORE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FAKE_STORE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FakeStoreTestRunner._test_control is not None:
            return FakeStoreTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FakeStoreTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FakeStoreTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FakeStoreTestRunner.load_test_control()
...
def load_env_local():
    FakeStoreTestRunner.load_env_local()


def env_override(m):
    return FakeStoreTestRunner.env_override(m)


def entity_data(v):
    return FakeStoreTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FakeStoreTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FakeStoreTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FakeStoreTestRunner.load_test_control()


def live_client_options():
    return FakeStoreTestRunner.live_client_options()


def live_delay_ms():
    return FakeStoreTestRunner.live_delay_ms()
