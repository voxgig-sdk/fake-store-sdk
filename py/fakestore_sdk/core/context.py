...
from fakestore_sdk.core.helpers import get_ctx_prop, to_map
...
class FakeStoreContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, FakeStoreSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, FakeStoreResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, FakeStoreResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return FakeStoreError(code, msg, self)
