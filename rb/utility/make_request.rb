...
module FakeStoreUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FakeStoreResponse.new({})
    ...
    if fetch_err
      ...
      response = FakeStoreResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FakeStoreResponse.new(fetched)
    else
