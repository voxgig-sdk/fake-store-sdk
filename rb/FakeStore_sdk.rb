...
require_relative 'FakeStore_types'
...
class FakeStoreSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FakeStoreHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FakeStoreHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FakeStoreFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FakeStoreUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FakeStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FakeStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FakeStoreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FakeStoreError.new(
        "#{op}_allow",
        "FakeStoreSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FakeStoreHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FakeStoreError.new(
        "graphql_error", "FakeStoreSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
