...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FakeStoreOmni.make_runner(TEST_JSON_FILE, FakeStoreSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FakeStoreOmni.errify(err)
    end
