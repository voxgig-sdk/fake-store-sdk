...
class UserEntityTest < Minitest::Test
  def test_create_instance
    testsdk = FakeStoreSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = FakeStoreSDK.test(seed, nil)
    seen = base.User(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = FakeStoreConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = FakeStoreSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FAKE_STORE_TEST_USER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def user_basic_setup(extra)
  ...
  entid_env_raw = ENV["FAKE_STORE_TEST_USER_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["FAKE_STORE_TEST_USER_ENTID"])
